...
function AttachmentRow({ att, taskPath, onDelete }) {
  return (
    <div className="flex items-center gap-3 p-2.5 bg-white/[0.02] border border-white/5 rounded-xl group hover:border-white/10 transition-all">
      <div className="p-1.5 bg-dark-700/50 rounded-lg shrink-0">
        <Paperclip size={14} className="text-slate-400" />
      </div>
      <div className="flex-1 min-w-0">
        <p className="text-xs font-semibold text-slate-200 truncate">{att.name || att.url}</p>
        <p className="text-[10px] text-slate-500 truncate">{att.url}</p>
      </div>
...
export default function TaskDetailPanel({ task, onClose, onUpdate, onCreate, onDelete, members = [] }) {
  ...
  useEffect(() => {
    ...
    return onSnapshot(q, snap => {
      ...
      setSubtasks(data);
      if (data.length > 0) setShowSubtasks(true);
    });
  }, [taskPath]);
  ...
  const handleCreate = () => {
    ...
    if (onCreate) {
      const newTaskData = { title: title.trim(), status, priority, description, assignedTo: assignees, startDate, dueDate };
              ...
              <div className="px-10 py-8 flex flex-col gap-8 max-w-4xl mx-auto w-full">
                ...
                <div className="space-y-4">
                  ...
                  <textarea
                    ...
                    className="w-full bg-white/[0.01] hover:bg-white/[0.02] border border-white/5 hover:border-white/10 rounded-2xl px-5 py-4 text-base text-slate-300 outline-none focus:border-primary-500/50 transition-all resize-y placeholder:text-slate-600 min-h-[120px]"
                  />
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  ...
                  <motion.button whileHover={{ scale: 1.02, y: -2 }} whileTap={{ scale: 0.98 }}
                    onClick={() => { setShowSubtasks(true); setTimeout(() => document.getElementById('subtask-input')?.focus(), 100); }}
                    ...
                    <div className="flex flex-col">
                      <span className="text-sm font-bold text-slate-200 group-hover:text-white">Add Subtask</span>
                  ...
                  {/* Add Attachment */}
                  <motion.button whileHover={{ scale: 1.02, y: -2 }} whileTap={{ scale: 0.98 }}
                    onClick={() => { setShowAttachments(true); setTimeout(() => document.getElementById('attach-url-input')?.focus(), 100); }}
                    ...
                    <div className="p-2 rounded-xl bg-pink-500/10 text-pink-400"><FileUp size={16} /></div>
                    <div className="flex flex-col">
                      <span className="text-sm font-bold text-slate-200 group-hover:text-white">Attachment</span>
                      <span className="text-[10px] font-medium text-slate-500">
                        {attachments.length > 0 ? `${attachments.length} file${attachments.length > 1 ? 's' : ''}` : 'Files & Links'}
                      </span>
                    </div>
                  </motion.button>
                  ...
                  <motion.button whileHover={{ scale: 1.02, y: -2 }} whileTap={{ scale: 0.98 }}
                    ...
                    <div className="p-2 rounded-xl bg-amber-500/10 text-amber-400"><Link2 size={16} /></div>
                    <div className="flex flex-col">
                ...
                <AnimatePresence>
                  {showSubtasks && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
                      ...
                      {!isNewTask && (
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            className="flex-1 bg-dark-800/50 border border-white/5 focus:border-primary-500/50 rounded-xl px-3 py-2 text-sm text-white outline-none placeholder:text-slate-600 transition-all"
                          />
                          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                      ...
                      {isNewTask && (
                        <p className="text-[11px] text-slate-500 italic text-center">Save task first to add subtasks.</p>
                      )}
                    </motion.div>
                  )}
                </AnimatePresence>
                  ...
                  {showAttachments && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <button onClick={() => setShowAttachments(false)} className="text-slate-600 hover:text-slate-400 transition-colors">
                          <X size={14} />
                        </button>
                      </div>
                      ...
                      {attachments.length > 0 && (
                        <div className="space-y-2">
                          {attachments.map(att => (
                            <AttachmentRow key={att.id} att={att} taskPath={taskPath} onDelete={handleDeleteAttachment} />
                          ))}
                        </div>
                      )}
                      ...
                      {!isNewTask && (
                        <div className="p-3 bg-white/[0.02] rounded-xl border border-white/5 space-y-2">
                          <input
                            ...
                            className="w-full bg-dark-800/50 border border-white/5 focus:border-primary-500/50 rounded-xl px-3 py-2 text-sm text-white outline-none placeholder:text-slate-600 transition-all"
                          />
                          <div className="flex items-center gap-2">
                            <input
                              value={attachName}
                            ...
                            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                              ...
                              className="px-4 py-2 bg-pink-600 hover:bg-pink-500 disabled:opacity-40 text-white text-xs font-bold rounded-xl transition-all flex items-center gap-1.5">
                              {addingAttachment ? <Loader2 size={12} className="animate-spin" /> : <Paperclip size={12} />} Attach
                            </motion.button>
                          </div>
                        </div>
                      )}
                      {isNewTask && (
                        <p className="text-[11px] text-slate-500 italic text-center">Save task first to add attachments.</p>
                      )}
                    </motion.div>
                  )}
                </AnimatePresence>
