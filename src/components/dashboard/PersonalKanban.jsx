...
import { usePersonalKanban } from '../../hooks/usePersonalKanban';

export default function PersonalKanban() {
    const { columns, loading, updateColumns, addTask } = usePersonalKanban();
    ...
    const handleAddCard = async (e, colId) => {
        ...
        // Add task mapping down through global tasks store and Firestore
        await addTask(newCardTitle, colId);
    ...
    return (
        <div className="h-full flex flex-col">
            <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-bold text-white">Personal Board</h2>
                                            ...
                                            <Draggable key={card.id} draggableId={card.id} index={index}>
                                                {(provided, snapshot) => (
                                                    ...
                                                    >
                                                        ...
                                                        {card.desc && <p className="text-xs text-slate-400 line-clamp-2">{card.desc}</p>}
                                                    </div>
                                                )}
                                            </Draggable>
                                        ))}
                                        ...
                                        {isAddingCard === col.id ? (
                                            <form onSubmit={(e) => handleAddCard(e, col.id)} className="mt-2">
                                                ...
                                                <div className="flex gap-2 mt-2">
                                                    <button type="submit" className="px-3 py-1 bg-primary-600 text-white text-xs rounded-md hover:bg-primary-500 font-medium">Add Card</button>
